...
Основная проблема состоит в том, что непонятно, как работать с $B_1,\ldots, B_p$ так, чтобы $C\cdot F(B_1,\ldots, B_p): \Cql(\frakI)\rightarrow \Cql(R)$. Решением этой проблемы мы сейчас и займёмся.

Во-первых, введём понятие декартова произведения функций и диагонализации:
\begin{definition}
...
Если 
\[
\forall r=\{1,\ldots, s\}\ B_k^r: \Cql(\frakI)\rightarrow\Cql(R);\ B_k^r\in\frakM^0,
\] 
тогда
\[
