...
\end{St}
\begin{proof}
Категория $\Sigma$~--- подкатегория $\Sigma_0$. Если категория имеет полную подкатегорию, то она полна. 
\end{proof}

\begin{St}
При $\forall \frakU$, $|\frakU| \geqslant 2 \ \exists p \in \mathbb{N}$ и $\exists \Cql^p(\frakU), (\hat{u}_0^1, \ldots, \hat{u}_0^p)$ и $\forall s_0 \in \sigma_0 \setminus \{e\}$, $s_0(\hat{u}_0^1, \ldots, \hat{u}_0^p) \neq (\hat{u}_0^1, \ldots, \hat{u}_0^p)$ равносильно $\exists k = \{1, \ldots, p\}: \ s_0(\hat{u}_0^k) \neq \hat{u}_0^k$.
\end{St}
